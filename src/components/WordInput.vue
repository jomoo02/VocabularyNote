...
const outFocusInput = (e) => {
  if (
    ...
    !e.target.classList.contains('inputTag')
  ) {
...
<template>
  <div>
    <div
      class="h-9 px-3 border flex items-center bg-white border-black inputTag"
      ref="input_container"
    >
      <button v-show="inputWord.length > 0" @click="inputWordClear">
        <Icon icon="ph:x-bold"></Icon>
      </button>
      <input
        ref="wordSearch_input"
        placeholder="단어를 입력해주세요"
        :value="inputWord"
        @keyup.enter="wordSearch(inputWord)"
        @focus="recentWordFocus = true"
        @input="(event) => (inputWord = event.target.value)"
        class="w-full px-2 focus:outline-0 inputTag"
      />
      <button ref="search_btn" class="inputTag" @click="wordSearch(inputWord)">
        <Icon icon="ion:search" width="24" height="24" />
      </button>
    </div>
    <!-- 최근 검색한 단어 -->
    <div
      v-show="recentWordFocus"
      class="absolute w-full bg-white border-[1.5px] border-black inputTag"
    >
      <div
        class="text-[12px] text-slate-500 font-semibold px-2 py-0.5 inputTag"
      >
        최근 검색한 단어
      </div>
      <div
        ...
        :key="index"
        class="flex items-center justify-between px-2 inputTag"
      >
        <button>
          <span @click="wordSearch(word)" class="text-sm inputTag">{{
            word
          }}</span>
        </button>
        <button @click="recentWordDeleteClick(index)" class="flex inputTag">
          <Icon icon="ph:x"></Icon>
        </button>
      </div>
    </div>
