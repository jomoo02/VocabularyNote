...
import ModalWordInfo from './ModalWordInfo.vue';
...
async function recentWordSelect(word) {
    wordSearch_input.value.disabled = true;
    const data = await axios.get(`/search/language/v1/search.json?cate=lan&q=${word}`);
    const searchWordMean = [];
    data.data.items.lan.forEach(item => {
        searchWordMean.push(item.item.split('|'));
    });
    ...
    <div class="fixed top-0 z-20 flex w-full justify-between py-2 px-10 md:px-24 lg:px-48 2xl:px-80  bg-[#2E4559] h-16 min-h-16 max-h-16 items-center">
        ...
        <div class="w-1/2 relative">
            <div class="h-9 px-3  border flex items-center bg-white border-green-500  relative inputTag">
                <button v-show="inputWord !== ''" @click="inputWordClear"><Icon icon="ph:x-bold"></Icon></button>
                <input ref=wordSearch_input placeholder="단어를 입력해주세요" v-model="inputWord" @keyup.enter="wordSearch" @focus="focusInput" 
                class="w-full px-2  focus:outline-0 inputTag"/>
            ...
            <div v-if="recentWordFoucs" class="absolute w-full bg-white  inputTag border border-green-500">
                ...
                <div v-for="word in store.recentWordsDic" class=" px-2 ">
                    <span @click="recentWordSelect(word)" class="cursor-pointer text-sm">{{ word }}</span>
                </div>
            </div>
        </div>

        <!-- btn -->
        <div class="" @click="store.contentChange">
        ...
        <!-- <button @click="store.modal = true">btn</button> -->
        <Teleport to="body" >
            <ModalWordInfo v-if="store.inputModal" :word="store.word" :means="store.means"></ModalWordInfo>
        </Teleport>
